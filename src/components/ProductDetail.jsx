    ...
    <>
      ...
      <div className="pd-page-wrapper">
        <ProductView
          ...
          siblings={data.items.filter((p) => p.id !== product.id)}
        />
      </div>
      <Footer />
    </>
...
function ProductView({ product, category, categoryName, siblings }) {
  ...
  return (
    <div className="pd-page">
      <SEO
        title={product.name}
        description={`${product.name} : ${product.description} Vente en gros à Témara, livraison partout au Maroc.`}
        url={`https://immi-negoce.ma/produits/${category}/${product.id}`}
      />
      ...
      <div className="pd-layout">
        ...
        <div className="pd-info">
          ...
          <p className="pd-desc">
            Disponible en gros chez IMMI Négoce à Témara, avec livraison partout au Maroc.
            Demandez votre devis gratuit ou commandez directement sur WhatsApp.
          </p>
          ...
          {siblings.length > 0 && (
            <div className="pd-siblings">
              ...
              <div className="pd-siblings-list">
                {siblings.map((p) => (
                  <Link key={p.id} to={`/produits/${category}/${p.id}`}>{p.name}</Link>
                ))}
              </div>
            </div>
          )}
